...
CHANNELS = {
    ...
    "weekly": {
        ...
        "project_ids": ["222913532", "222913092"],
        "keywords": {
            "概念": ("概念推导", "W01"),
            "推导": ("概念推导", "W01"),
            "KOTO": ("概念推导", "W01"),
...
def build_channel(channel_key, channel_config):
    ...
    for proj_id in channel_config["project_ids"]:
        try:
            ...
            print(f"  Warning: project {proj_id} failed: {e}")
            continue
        for f in files:
...
def sync():
    print("=" * 55)
    print("  CFH Design Center - Multi-channel Sync")
    print("=" * 55)
    output_channels = {}
    for ch_key, ch_config in CHANNELS.items():
        ...
        print(f"  OK {ch_data['totalDocuments']} files")
    now = datetime.now(CST).strftime("%Y-%m-%d %H:%M:%S")
    total_all = sum(c["totalDocuments"] for c in output_channels.values())
    js_content = (
        "// CFH Design Index Data | Multi-channel format\n"
        f"// Updated: {now}\n"
        f"// Total: {total_all} files | {len(output_channels)} channels\n\n"
        "var DOC_DATA = {\n"
        '  "channels": {\n'
    )
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as fp:
        fp.write(js_content)
    print("=" * 55)
    print(f" Done! Total: {total_all} files -> {OUTPUT_FILE}")
    print("=" * 55)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CFH multi-channel sync")
